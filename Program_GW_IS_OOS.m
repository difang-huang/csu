...
%% plot CSU and excess stock return

% DATE
time  =   datetime(1996,1,31):calmonths(1):datetime(2019,08,31); time = time';  
x = time;
...
exportgraphics(gca, 'figure1a.eps')  
...
recessionplot;
set(0,'DefaultLegendAutoUpdate','off')
exportgraphics(gca, 'figure1b.eps')      
...
for p=1:P;
    ...
    for j=1:length(h);
        ...
        FC_PR(p,size(GW,2)+1,j)=[1 SII(R+(p-1))]*results_SII_j_p.beta;
